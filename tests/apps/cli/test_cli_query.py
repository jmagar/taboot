...
@pytest.mark.unit
def test_query_command_requires_question() -> None:
...
@pytest.mark.unit
def test_query_command_accepts_filters() -> None:
...
@pytest.mark.slow
def test_query_command_with_real_services(qdrant_client, neo4j_client) -> None:
    """Test query command against real services."""
    result = runner.invoke(app, ["query", "Which services expose port 8080?", "--top-k", "5"])
